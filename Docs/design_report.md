...
$$
m_{hub} = 750 kg \quad (1)
$$

$$
I_{hub} = \begin{bmatrix} 900 & 0 & 0 \\ 0 & 800 & 0 \\ 0 & 0 & 600 \end{bmatrix} \text{ kg} \cdot \text{m}^2 \quad (2)
$$
...
$$
I_{xx}\dot{\omega}_x \;-\; (I_{yy}-I_{zz})\,\omega_y\omega_z \;=\; \tau_x \quad (4)
$$

$$
I_{yy}\dot{\omega}_y \;-\; (I_{zz}-I_{xx})\,\omega_z\omega_x \;=\; \tau_y \quad (5)
$$

$$
I_{zz}\dot{\omega}_z \;-\; (I_{xx}-I_{yy})\,\omega_x\omega_y \;=\; \tau_z \quad (6)
$$
...
$$
\boldsymbol{\sigma} = \hat{\mathbf{e}} \tan\left(\frac{\Phi}{4}\right) \quad (7)
$$
...
$$
\dot\sigma = \frac{1}{4} B(\sigma)\omega \quad (8)
$$

Where:

$$
B(\sigma)=(1-\sigma^2)I_3 + 2[\sigma_\times]+2\sigma\sigma^T \quad (9)
$$

and as we surely recall from vector algebra lectures that:

$$
\sigma^2 = \sigma^T\sigma \quad (10)
$$
...
$$
\mathbf{G}_s = \begin{bmatrix} | & | & | \\ \hat{\mathbf{g}}_{s1} & \hat{\mathbf{g}}_{s2} & \hat{\mathbf{g}}_{s3} \\ | & | & | \end{bmatrix} \quad (15)
$$
...
$$
G_s = \begin{bmatrix} + sin(\alpha) & - sin(\alpha) & 0 \\ + cos(\alpha) & + cos(\alpha) & 0 \\ 0 & 0 & 1 \end{bmatrix} \quad (16)
$$
...
$$
\mathbf{G}_s = \begin{bmatrix} \frac{\sqrt{2}}{2} & -\frac{\sqrt{2}}{2} & 0 \\ \frac{\sqrt{2}}{2} & \frac{\sqrt{2}}{2} & 0 \\ 0 & 0 & 1 \end{bmatrix} \quad (17)
$$
...
$$
\det(\mathbf{G}_s) = 1 \cdot \det\begin{bmatrix} \frac{\sqrt{2}}{2} & -\frac{\sqrt{2}}{2} \\ \frac{\sqrt{2}}{2} & \frac{\sqrt{2}}{2} \end{bmatrix} = 1 \cdot \left(\frac{1}{2} + \frac{1}{2}\right) = 1 \quad (18)
$$
...
$$
\ddot r_{abs} = \dot \omega \times r + \omega \times (\omega \times r ) \quad (19)
$$
...
$$
\dot \omega \times r = \begin{bmatrix} 0 \\ 0 \\ \dot \omega_z \end{bmatrix} \times \begin{bmatrix} 0 \\ r_y \\ 0 \end{bmatrix} =  \begin{bmatrix} -\dot \omega_z r_y \\ 0 \\ 0 \end{bmatrix} \quad (20)
$$

And the centripetal acceleration is:

$$
\boldsymbol{\omega} \times (\boldsymbol{\omega} \times \mathbf{r}) = \begin{bmatrix} 0 \\ -\omega_z^2 r_y \\ 0 \end{bmatrix} \quad (21)
$$
...
$$
m\ddot\rho + c \dot \rho + k \rho = F_{excitation} \quad (22)
$$
...
$$
F_{excitation} = -m \cdot a_{tangential} = -m \cdot (-\dot{\omega}_z r_y) = m \cdot r_y \cdot \dot{\omega}_z \quad (23)
$$
...
$$
m\ddot{\rho} + c\dot{\rho} + k\rho = \frac{m \cdot r_y}{I_{zz}} \cdot \tau_z \quad (24)
$$

Dividing by $m$ gives the standard second order form:

$$
\ddot{\rho} + 2\zeta\omega_n\dot{\rho} + \omega_n^2\rho = \frac{r_y}{I_{zz}} \cdot \tau_z \quad (25)
$$

This reveals the modal gain:

$$
G_{modal} = \frac{r_y}{I_{zz}} \quad (26)
$$
...
$$
t_s \approx \frac{4}{\zeta \omega_n} \quad (27)
$$

For Mode 1 ($\zeta = 0.02$, $\omega_n = 2.51$ rad/s):

$$
t_s \approx \frac{4}{0.02 \times 2.51} = \frac{4}{0.05} = 80 \text{ seconds} \quad (28)
$$
...
$$
sI_{zz}\Omega(s) = T(s) \quad (29)
$$

$$
s\Sigma(s) = \frac{1}{4}\Omega(s) \quad (30)
$$

Eliminating $\Omega(s)$:

$$
\Sigma(s) = \frac{1}{4s}\Omega(s) = \frac{1}{4s} \cdot \frac{T(s)}{sI_{zz}} \quad (31)
$$

$$
G_{rigid}(s) = \frac{\Sigma(s)}{T(s)} = \frac{1}{4I_{zz}s^2} \quad (32)
$$
...
$$
\frac{P_i(s)}{T(s)} = \frac{G_{modal,i}}{s^2 + 2\zeta_i\omega_{n,i}s + \omega_{n,i}^2} \quad (33)
$$

The complete plant from torque to attitude includes the coupling:

$$
G_{flex}(s) = \frac{1}{4I_{eff}s^2} \cdot \prod_{i=1}^{N_{modes}} \frac{s^2 + 2\zeta_i\omega_i s + \omega_i^2 + \Delta_i}{s^2 + 2\zeta_i\omega_i s + \omega_i^2} \quad (34)
$$
