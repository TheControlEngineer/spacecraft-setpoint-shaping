...
# Add parent paths for input_shaping import
...
if __name__ == "__main__":
    ...
    # Save to data/trajectories/ directory
    save_dir = os.path.join(os.path.dirname(__file__), "..", "..", "data", "trajectories")
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, "spacecraft_trajectory_4th_180deg_30s.npz")
    np.savez(save_path,
             time=t, theta=theta, omega=omega, alpha=alpha,
             jerk=jerk, snap=snap, trajectory_info=traj_info)
    print(f"Saved: {save_path} (Corrected Algorithm)")
